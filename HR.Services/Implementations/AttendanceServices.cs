...
namespace HR.Services.Implementations
{
    public class AttendanceService : IAttendanceServices
    {
        ...
        #region Clock In
        public async Task<string> ClockIn(ClockInDTO clockInDto)
        {
            // Check if the employee exists
            var existUser = await _userManager.FindByIdAsync(clockInDto.EmployeeId);
            if (existUser == null) return "Employee does not exist.";
            ...
            {
                return "Employee has already clocked in today.";
            }
        ...
        #region Clock Out
        public async Task<string> ClockOut(ClockOutDTO clockoutDTO)
        {
            ...
            if (attendance == null)
            {
                return "Clock-in record not found. Please clock in first.";
            }

            // Check if clock-out time is already set
            if (attendance.ClockOutTime != null)
            {
                return "Employee has already clocked out.";
            }
        ...
        public async Task<IEnumerable<AttendanceRecordDTO>> GetDailyAttendanceAsync(DateOnly date)
        {
        ...
        public async Task<string> updateAttendanceAsync(UpdateAttendanceDTO record)
        {
            ...
            {
                return "Clock-out time cannot be earlier than clock-in time.";
            }
        ...
        public async Task<string> DeleteAttendanceAsync(string EmployeeID)
        {
